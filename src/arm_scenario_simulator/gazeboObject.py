...
def spawn_sdf(gazebo_name, path_to_sdf, position, orientation, static):
    try:
        ...
        if resp_spawn.success:
            ...
            rospy.logerr(
                "Could not spawn " + path_to_sdf + " (from " + path_to_sdf + "), status : " + resp_spawn.status_message)
            return False
...
class GazeboObject:
    ...
    def set_state(self, position, orientation=None, linear_twist=None, angular_twist=None, reference_frame="world"):
        ...
        """
        message = ModelState(self.gazebo_name, Pose(position, orientation), Twist(linear_twist, angular_twist),
                             reference_frame)
    ...
    def get_state(self, reference_frame="world"):
        ...
        if not resp_get.success:
            raise Exception("Could not get state of " + self.gazebo_name + " , status : " + resp_get.status_message)
        else:
            return resp_get
